    ...
    void Start()
    {
        ...
        for (int j = 0; j < m_y; j++)
        {
            for (int i = 0; i < m_x; i++)
            {
                ...
                xOffset = boxCollider2d.size.x * plate.transform.localScale.x;
                yOffset = boxCollider2d.size.y * plate.transform.localScale.y;

                plate.transform.localPosition = new Vector3(i * xOffset, j * yOffset);
